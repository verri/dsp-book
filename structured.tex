...
Note that the idea behind the definition of BCNF and 4NF are slightly different from the
...
\begin{tablebox}[label=tab:messy3]{Messy table, adapted from airquality dataset, where variables are stored in both rows and columns.}
  ...
  \begin{tabular}{llllcccc}
    \toprule
    \textbf{id} & \textbf{year} & \textbf{month} & \textbf{element} & \textbf{d1} & \textbf{d2} & \textbf{\dots} & \textbf{d31} \\
    \midrule
...
names), as seen in \cref{tab:tidy3a}.  Afterwards, we widen the table in function of their names.  Finally, we remove
