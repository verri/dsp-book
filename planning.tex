...
Let us say our goal is to reach a certain performance threshold $\rho_0$.  After an
experiments done with $10$ repeated $10$-fold cross-validation, we have the average
performance $\bar{\rho}$ and the standard deviation $\sigma$.  If $\bar{\rho} - \sigma \gg
\rho_0$, it is very likely that the solution will reach the threshold in production.
...
\textcite{Benavoli2017} propose an interesting technique to estimate the density function
$p(\rho)$ taking into account the training dependency in the cross-validation.  They
