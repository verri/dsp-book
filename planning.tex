...
a sampling strategy to generate $r$ datasets $D_1, D_2, \ldots$ from $D$.  Each
...
different result for each dataset $D_i$ used as an input.  In other words, the parameters
...
These parameters, $\left[\phi_i, \theta_i\right]$ are the solution $M_i$, and must be
calculated exclusively using the training set in $D_i$.

\paragraph{Prediction} \dots

\paragraph{Performance assessment} \dots

\paragraph{Analysing the results} \dots

% TODO: move below and talk about concatenating out of sample predictions, and correlation
% between the predictions in different folds.
