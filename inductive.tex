...
For the examples in the following subsections, we consider the dataset for the XOR
problem --- see \cref{tab:xor}.
...
linearly separable --- $\vec{w} \cdot \vec{x} > 0$, where $\vec{x} = [1, x_1, x_2]$ is
just the equation of a hyperplane.
...
and the perceptron model fails to classify the dataset correctly.
