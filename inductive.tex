...
% Let the outcome $y$ be a real value and
...
% metric $l_2$.
% In other words, we look for $\theta'$ such that
% \begin{equation*}
...
Assuming that the distributions $\Prob(x)$ and $\Prob(x\mid y)$ are not only fixed but
...
However, since
\[
  \Prob(y \mid x) = \frac{\Prob(x \mid y) \Prob(y)}{\Prob(x)}\text{,}
\]
the Bayes classifier can be rewritten as
\begin{equation*}
  f_\text{Bayes}(x) = \argmax_{y \in \mathcal{Y}} \Prob(x \mid y) \Prob(y)\text{,}
\end{equation*}
as the denominator $\Prob(x)$ is the same for all classes.
By our premises, both distributions $\Prob(x \mid y)$ and $\Prob(y)$ are known.

% TODO: a proof

The \emph{Bayes error rate}, or irreducible error, is the lowest possible loss for any
classifier in a given problem.  The Bayes error rate is the probability of classification
error of the Bayes classifier.

Given that $b(x) = \Prob(y=1 \mid x)$, the Bayes error rate sums the errors of the
...
% A figure with two Gaussian curves, one for each class, that intersect each other.  The
% Bayes classifier is the line that separates the two classes, and the Bayes error is the
% area under the curve that is not covered by the Bayes classifier.
