...
classes.  It is a parametric model and the learning process minimizes the empirical risk
by adjusting its fixed set of parameters.

\begin{defbox}{Parametric model}{parametric}
  If the learning machine generates a set of functions $f_\theta$ where $|\theta|$ is
  always fixed, the models are called \emph{parametric}.  Parametric models are usually
  simpler and faster, but they are less flexible.  In other words, it is up to the
  researcher to choose the best model ``size'' for the problem.  If the model is too
  small, it will not be able to capture the complexity of the data.  If the model is too
  large, it will be too complex, too slow to train and might overfit to the data.
  Note, however, that the VC dimension and number of parameters are not the same thing ---
  consult \cref{sub:vc-dimension-intuition}.
\end{defbox}
...
\begin{equation*}
  ...
  \end{cases}
\end{equation*}
The parameters $\vec{w}$ are called the weights of the perceptron.
The equation $\vec{w} \cdot \vec{x} = 0$, where $\vec{x} = [1, x_1,
x_2]$, is the equation of a hyperplane.

\begin{figurebox}[label=fig:perceptron]{Perceptron decision boundaries.}
  \centering
  \begin{tikzpicture}
    \begin{axis}[
        axis x line=bottom,
  ...
  \end{tikzpicture}
  \tcblower
  The perceptron learning assumes that the classes are linearly separable.
