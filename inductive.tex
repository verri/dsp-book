...
\end{equation*}
where $I_{\cdot}$ is the indicator function that returns one if the condition is
true and zero otherwise.  Let $b(x) = \Prob(y = 1 \mid x)$, we have that
\[
  \int I_{f(x) \neq y}\, d\!\Prob(y | x) =
    b(x) I_{f(x) = 0} + (1 - b(x)) I_{f(x) = 1}\text{,}
\]
...
\begin{equation*}
  R_\text{Bayes} = \int \left[
    b(x) I_{f_\text{Bayes}(x) = 0} +
    \left(1 - b(x)\right) I_{f_\text{Bayes}(x) = 1}
  \right] d\!\Prob(x)\text{.}
\end{equation*}
...
\begin{figure}
  ...
  \end{tikzpicture}
  \caption{
  Unexplained variance is the error of the regression.
  ...
  \label{fig:explained-unexplained-variance}
\end{figure}
...
\begin{figure}
  ...
  \end{tikzpicture}
  \caption{
  High-frequency sine wave functions have infinite VC dimension.
  They approximate well any set of points even though they may come
...
\begin{definition}{Parametric model}\label{def:parametric}
...
\begin{figure}
  ...
  \end{tikzpicture}
  \caption{
  Perceptron decision boundaries in the AND dataset.
...
\begin{table}
  ...
  \end{tabular}
  \caption{
  Truth table for the predictions of the perceptron.
...
\begin{figure}
  ...
  \end{tikzpicture}
  \caption{
  Angle between $\vec{w}$ and $\vec{x}$ in a posite output.
...
\begin{figure}
  ...
  \end{tikzpicture}
  \caption{
  Angle between $\vec{w}$ and $\vec{x}$ in a negative output.
...
pressuring their weights to zero.}.
...
\begin{figure}
  ...
  \end{tikzpicture}
  \caption{
  Maximal margin classifier for the AND dataset.
...
Moreover, since the number of parameters of the maximal margin classifier depend on the
