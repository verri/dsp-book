...
\begin{figure}
    ...
    \begin{tikzpicture}[%
      ...
      \node[align=center, label=below:product owner] (po) at (0, 0) {\faUserTie};
      \node[align=center, label=below:product backlog] (backlog) at (0, -1.5) {\faListOl};

      \node[align=center, label=below:dev team] (dev) at (3, 0) {\faUsers};
      \node[align=center, label=below:sprint backlog] (sbacklog) at (3, -1.5) {\faClipboardList};
      ...
      \draw[-Stealth, very thick] (sbacklog.east)
        ...
        to[out=0, in=0] (5, 0.5)
        to[out=180, in=130] (5, -1.4);

      \node[font=\footnotesize] at (5.2, -0.5) {sprint};

      % Scrum master on top of sprint
    ...
    \end{tikzpicture}
    \caption{
      Scrum framework overview.  The development centers around the sprints, which are time-boxed
      iterations.  Roles, other ceremonies, and artifacts support the sprints and ensure the
      progress and quality of the product.
    }
    \label{fig:scrum}
\end{figure}
