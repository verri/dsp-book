...
discussed in that chapter.  Exploration also refers to \emph{exploratory
...
preprocessing ---
adjustments to the data to be used by the machine learning algorithms ---, \cref{chap:slt}
...
``collect/manage data'' tasks more carefully.  Data handling operations that do not
depend on the values of the dataset (sample independent) are performed in the data sprint,
while operations that depend on the values of the dataset are performed together with
modeling.

This approach not only improves reliability of the solution validation, but also improves
maintenance of the solution.  (Not surprisingly, frameworks like \texttt{scikit-learn} and
\texttt{tidymodels} have an object that allows the user to combine data preprocessing and
model training.)
