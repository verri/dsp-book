...
\begin{figurebox}[label=fig:zumel]{Diagram of the data science process proposed by \textcite{Zumel2019}.}
  ...
  \begin{tikzpicture}
    ...
    \path [line, <->] (dg) to[out=0, in=90] (cm.north);
    \path [line, <->] (cm) -- (bm);
    \path [line, <->] (bm.south) to[out=-90, in=0] (em.east);
    \path [line, <->] (em.west) to[out=180, in=-90] (pd.south);
    \path [line, <->] (pd) -- (dm);
    \path [line, <->] (dm.north) to[in=180, out=90] (dg.west);
  \end{tikzpicture}
...
\begin{itemize}
  \item CRISP-DM is too focused on the data;
  \item Nina and Zumel's approach is too focused on the roles;
  \item SCRUM is too focused on the software development.
\end{itemize}
