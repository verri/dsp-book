...
to find the best solution, i.e. the combination of preprocessing and model that best
solves the problem.
...
Note that the preprocessing pipeline and the models themselves\footnote{The learned
...
During sprints, the deliverables of data exploration are
...
Do not fall on the trap of forever improving the model.
...
During model validation, use your own
...
If possible setup feedback from the user
interface.  Avoid automation of model releases because concept drift usually requires
exploratory analysis.

\paragraph{Use the appropriate backend}

REST API vs websocket.  The choice depends on the
