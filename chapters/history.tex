...
enables us to understand the field's evolution and guide its future development.  Besides, history also teaches us ways to avoid repeating the same mistakes.
...
sometimes avoiding directions that are not relevant in the context of \gls{inductivelearning}.
...
The term ``data science'' itself was coined in the 1960s by Peter Naur (/naʊə/). He was
...
science.  In the 1960s, the subject was practiced in Denmark under Peter
...
departs from the concept that \emph{data} is ``a representation of facts or ideas in a
...
learning, data visualization and data analysis for multidisciplinary studies.
...
is shown in \cref{fig:cleveland}.  Note that Cleveland never defined an
...
Be aware that the scientific literature has no consensus on the definition of data science, and it is still considered
...
In \cref{chap:data}, I dare to provide yet another definition for the term.  I
...
Middle Eastern world, there are no significant advances in \gls{structureddata} collection
until the \nth{19} century\footnote{Research on Eastern timelines is left for future works.}.

Florence Nightingale (1820 -- 1910) is another major influential figure in the history of
data collection.  She was a passionate statistician
...
unstructured, semi-structured, and \gls{structureddata}.  They are highly scalable and
...
\textcite{Hillis1985}'s\footfullcite{Hillis1985} thesis.
...
The devices can be cellphones, coffee makers, washing machines, headphones,
...
analysis is called \gls{summarystatistics}, which consists of describing data in terms
...
bears his name: Bayes' theorem\footfullcite{Bayes1763}.  The theorem is used to calculate conditional
...
Bayes' rule is the foundation of learning from evidence, since it allows us to
...
developed the method of least squares\footfullcite{Gauss1809} for calculating the orbit of
Ceres to minimize the impact of measurement error\footnote{The method was first published by
Adrien-Marie Legendre (1752 -- 1833) in 1805, but Gauss claimed in 1809 that he had been
using it since circa 1794.}.
...
war with France in the 1790s.  He invented several types of diagrams\footfullcite{Playfair1786}
between the 1780s and 1800s, such as the line, area, and bar chart of economic data, and
the pie chart and circle graph to show proportions.

\subsubsection{Learning from data}

In the twentieth century and beyond, new advances were made in the field of statistics.
The development of \glspl{learningmachine} enabled the development of new techniques for data
analysis.
...
discriminant analysis\footfullcite{Fisher1936},
...
introduced the famous Iris dataset.}.
...
The field --- which studies quantification, storage, and communication of information ---, was
originally established by the works of Harry Nyquist (1889 -- 1976) and Ralph Hartley
(1888 -- 1970) in the 1920s, and Claude Shannon (1916 -- 2001) in the
1940s\footfullcite{Shannon1948}.
...
transmitted.  The theory is used to quantify the amount of information in a dataset.
...
In 1951, Evelyn Fix (1904 -- 1965) and Joseph L. Hodges Jr. (1922 -- 2000) proposed the
k-nearest neighbors algorithm\footfullcite{Fix1951},
...
In the 1960s, a psychologist called Frank Rosenblatt (1928 -- 1971) developed the
perceptron\footfullcite{Rosenblatt1958}, the first model of a \gls{learningmachine}.  While the idea of a mathematical neuron was not new, he was the first
...
of the perceptron was a breakthrough in the field of \glspl{learningmachine}, drawing attention
...
\paragraph{Tukey's \glsfmtlong{eda}}

In 1977, John W. Tukey (1915 -- 2000), an American mathematician and statistician, published
``Exploratory Data Analysis''\footfullcite{Tukey1977}, formalizing a philosophy that
countered the dominance of confirmatory statistical methods.  Tukey advocated for an informal
study of data through graphical and summary techniques --- such as the box plot and the
stem-and-leaf display --- before any formal modeling.  His work reinforced the idea that data
analysis should not be limited to testing hypotheses, but should also include open-ended
exploration.

\paragraph{Empirical risk minimization principle}

Although many \glspl{learningmachine} were developed until the 1960s, they did not advance
...
theoretical framework to support the development of new \glspl{learningmachine}.

However, these years were not completely unfruitful.  As early as 1968, Vladimir Vapnik
(born 1936) and Alexey Chervonenkis (1938 -- 2014) developed the fundamental concepts of
\glsunset{vc}\gls{vc} entropy and \gls{vc} dimension for data classification problems.  As a result, a novel inductive
...
methods combine multiple \glspl{learningmachine} to improve the performance of the individual
machines.
...
\gls{learningmachine} based on the VC theory and the \gls{erm} principle.  Based on Cover's
...
The resulting method led to practical and efficient \glspl{learningmachine}.

\paragraph{Deep learning}

The idea of neural networks with multiple layers was around since the 1960s.
However, only in the late 2000s did the field of deep learning catch the attention of the scientific
...
\gls{transformer} model, a deep learning architecture based on attention mechanisms, in
contrast to recurrent or convolutional neural networks. These
advances --- despite limited theoretical progress --- enabled the development of \glspl{llm},
which gained significant public attention when made widely available by OpenAI in 2022.
...
Data science is a term that has been used in different ways by different people.
Independently of the definition, it is clear that data science is a field that has evolved
significantly over the years, and it is likely to continue evolving in the future.

Once we have a broad understanding of the history of data science, we can better
understand the current state of the field and its future directions.  In the next chapter,
I will provide a definition of data science and set up a framework for the rest of the
book.
