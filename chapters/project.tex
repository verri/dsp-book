...
project.
First of all, \emph{a data science project is a software project}.  The difference between data science
software and traditional software is that some components of the former are
...
Before discussing specific data science project methodologies, it is important to understand
what a project is.  According to the PMBOK\footnote{The PMBOK is a guide --- perhaps the
most important one --- of best practices for project management,
\fullcite{PMI2025}.}, a project is \emph{a temporary endeavor aimed at creating a unique result,
such as a product or service}.

The emphasis on the temporary nature of a project is important.  It means that a project
has a well-defined beginning and end.  Therefore, in this chapter, we will not deeply
discuss matters of operations, which include maintenance and monitoring.  These are the
continuous execution of activities
that occur after the product is built and delivered\footnote{This is where the terms
\gls{devops}, \gls{mlops}, and \gls{dataops} come from.}.  However, in organizations, projects and
operations coexist.  It is important that the project is conducted in a way that eases
the operation of the product, just as it is important that routine operations are flexible
enough to streamline project development.

Projects must also have a well-defined expected result.  This means that the project must
have a clear objective.  It must be measurable and achievable.  Unrealistic or poorly
defined objectives are one of the main causes of project failure.

Finally, projects usually require strong collaboration between teams, often within and
outside the organization --- much more so than operations do.  This is because they
typically involve specialized knowledge that is not available in a single team.

\section{CRISP-DM}

The \gls{crispdm}\footnote{Official guide available at
\href{https://www.ibm.com/docs/it/SS3RA7_18.3.0/pdf/ModelerCRISPDM.pdf}{ibm.com}.} is a
methodology for data mining projects developed in the 1990s by IBM, and it is still widely
used today.
...
Note that tasks beyond deployment, such as maintenance and monitoring, are not directly
addressed by the methodology but are delegated to the operations role.  Nevertheless,
\citeauthor{Zumel2019} emphasize the importance of interacting with operations personnel to
guarantee the sustainability of the solution, as well as the importance of documenting the
process and version controlling the code.

\subsection{Limitations of the ZM approach}

The approach is particularly interesting in consulting projects, where the data scientist
is not part of the organization, or even in internal projects of organizations with a
strongly hierarchical structure.  Like CRISP-DM, the ZM approach does not directly address
the software development aspects of the project.

Another criticism is the lack of a clear criterion for where the project ends and
operations begin.  The approach suggests that a data science project always develops just
a component, a pilot or demonstration, delegating the final version of the software to
another group.  However, in many modern organizations and due to current market demands,
data scientists are expected to build complete, production-ready software\footnote{With
the rapid pace of advances in data science and artificial intelligence, the
lifetime of software products is getting shorter.  However, this is not an excuse
not to build high-quality software.}.
...
code which usually contains the \gls{preprocessor}\footnote{Preprocessor is a fitted chain of
...
produce a dynamic dataset.  The solution search system is the software that employs \gls{datapreprocessing} and machine learning techniques, usually in a
\gls{hyperparameter}\footnote{\Glspl{hyperparameter} are parameters that are not fitted or learned
from the data, but rather set by the user.} optimization loop,
to find the best solution, i.e. the combination of \gls{preprocessor} and model that best
solves the problem.
...
Note that the \glspl{preprocessor} and the models themselves\footnote{The learned
...
code.  The same occurs with reports generated from exploration and validation
...
exploration report is updated for each version of the dataset.
...
validation methods such as \gls{crossvalidation} and Bayesian analysis --- refer to
...
If the execution instructions are simple, for example when
Docker\footnote{\url{https://www.docker.com/}} is used, clear instructions in a readme file
may suffice.

Pay attention to the dependencies between the dataset and
model training.  If the dataset changes significantly, not only the deployed \gls{preprocessor}
...
Although it may be worthwhile to manually enhance some important reports in terms of
presentation, during fast-paced sprint cycles, prefer using tools that generate reports
automatically. Quarto\footnote{\href{https://quarto.org/}{quarto.org}}, for instance, can
generate reproducible reports from version-controlled code.

\subsubsection{Set up quantitative goals}
...
strategies are more likely to find a good solution (\gls{preprocessor} and model).
...
For instance, a \gls{regressor} that predicts the probability of a binary outcome yields
different \glspl{classifier} depending on a threshold.  In this scenario, the user must be able
...
spokesman wants to be implemented.  Like in traditional Scrum, the items are ordered
...
Exploration also relates to \gls{eda}, a term coined by
\textcite{Tukey1977}\footfullcite{Tukey1977} to describe an informal study of data through graphical and summary
techniques.  His original proposal, however, was rooted in statistical modeling, not in
machine learning\footnote{This does not mean that all problems should be solved using
machine learning.  Statistical modeling is still a powerful tool for data analysis, with
advantages and disadvantages compared to machine learning.  The scope of this book covers
only the application of machine learning.}.  In fact, the two main goals of EDA are data
description and model formulation\footfullcite{Chatfield1986} --- the latter being delegated to
the learning algorithm in our context.  As a result, the data scientist should be careful
not to let excessive exploration bias the solution during the data sprint.
For a good introduction to a sensible \gls{eda}, which includes both understanding
...
\begin{defbox}{Data leakage}{}
  ...
  \gls{leakage} or bias in the validation phase.
\end{defbox}

\paragraph{Solution sprint}
The \emph{solution sprint} is focused on the solution search tasks: \gls{datapreprocessing}, machine
...
covers the subjects of \glspl{learningmachine} --- methods that can estimate an unknown function
...
which is the \gls{preprocessor} and the model.  The validation report is a document that
describes the expected performance of the solution in the real-world.  The script or
program that searches for the best pair of \gls{preprocessor} and model must be
...
modules (front-end, back-end, external tools), and infrastructure for the monitoring of
the solution in production.  Software documentation and unit tests are also part of this
sprint.
...
\begin{figurebox}[label=fig:our-sprints]{Tasks and results for each sprint types and their relationships.}
  ...
  \begin{tikzpicture}[%
    ...
    \node [block] (data-report) at (4, -0.5) {Exploration report};
...
algorithm is modified: the application must be updated to use the new \gls{preprocessor}
and model.
...
exploration report, performance reports, and the working software are ready for review.
...
\subsubsection{Monitoring and maintenance} Although monitoring and maintenance are
operations tasks, best efforts must be made to guarantee that the software is easy to
monitor and maintain.  For instance, the monitoring infrastructure must be
...
parameterized by the data values themselves are performed in the data sprint.  On the other hand,
...
tidymodels have an object that allows the user to combine \gls{datapreprocessing} and
model training.)
...
Finally, our approach emphasizes that the end result of the project must be a complete
software product, ready for deployment, ensuring the quality of both the code and the
data-driven solution, as well as their future maintainability.
