...
stage of data preparation, for example, we should never parameterize our data handling
...
\gls{splitinvariance} property, which ensures that the operations do not add bias to the data
...
In this section, I present a formal definition of \gls{structureddata}.  This definition is
...
over the bind operation.  This property is called \gls{splitinvariance}.
...
\Gls{splitinvariance} is a desirable property for data handling operations during the data
...
The reason is that \gls{splitinvariance} ensures that the operation does not depend on the
...
analysis\footnote{Note that \gls{splitinvariance} is a sufficient but not necessary condition
for preventing \gls{leakage}.  \Gls{splitinvariance} provides not only a ``safe by default''
guarantee, but also a way to pinpoint possible sources of \gls{leakage}.}.
...
\begin{tablebox}[label=tab:grades3]{Data table of student grades assuming student as the index.}
  \centering
  \rowcolors{2}{black!10!white}{}
  \begin{tabular}{cc|p{2.6cm}p{1.8cm}>{\raggedright\arraybackslash}p{1.2cm}}
    \toprule
    \textbf{s} & \textbf{student} & \textbf{subject} & \textbf{year} & \textbf{grade} \\
    \midrule
...
Imposing this restriction would complicate the data collection process, but it would
...
sources are in an observational unit different from the one we are interested in, after
...
operation reversible, we can parameterize it with a split column that indicates the source
of each row.
...
\begin{defbox}{Tagged split operation}{tagged-split}
  ...
  operation parameterized by $s$ creates disjoint tables $T_i = (K, H, c_i)$ that contain
...
rows\footnote{We consider a slightly different definition of \gls{splitinvariance} here, where
...
$T_0 = (K \cup \{\text{name}\}, H, c_0)$ and $T_1 = (K \cup \{\text{name}\}, H, c_1)$ disjoint tables,
\[
...
safe; nonetheless, a clear definition of \gls{splitinvariance} for binary operations is needed.
...
exploration and \gls{datapreprocessing}.  In these cases, other operations are needed.
...
for instance, in \gls{datapreprocessing} ---, we use variations of some operations in
\cref{sec:split-invariant-ops} that are not split-invariant.  These operations are
parameterized by the groups and the order of the rows.
...
Other notable efforts are the works of
\textcite{hutchison2017laradb}\footfullcite{hutchison2017laradb}, who proposes a
minimalist algebra of just three operators (extension, join, and union) that unifies
relational algebra and linear algebra computation, and
\textcite{petersohn2020towards}\footfullcite{petersohn2020towards}, who formalizes a
compact dataframe algebra of 14 operators reverse-engineered from pandas and demonstrates
its use in a scalable, parallel implementation.

However, in my opinion, the major deficiency of these works is that they mostly try to
...
also present the novel concept of \gls{splitinvariance}, which is a property that I think is
