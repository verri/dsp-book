...
The most common data format is \gls{structureddata}.  \Gls{structureddata} refers to
...
the understanding of \gls{structureddata}.
...
\textcite{Paul1993}\footfullcite{Paul1993} provides interesting insights about data types.  Although I do not
...
Moreover, the choice of data types is also related to the digital representation and the
domain of the data.  For example, consider a person's age.  Should it be an integer or a
floating-point number?  If integers, fractional years are irrelevant.  What is the valid
range?  Setting a maximum of 100 rejects supercentenarians; setting it too high allows
implausible values.  A model trained with certain bounds may behave unpredictably outside
them.
...
When we work with \gls{structureddata}, two concepts are very important: database normalization
...
\paragraph{Simple example}  Consider the relation $R[A, B, C, D]$ with primary key $A$ and
functional dependencies $A \to B,~B \to C,~C \to D$.  The relation is in 2NF because,
with a single-attribute primary key, partial dependency is impossible.  However, it is not
in 3NF because $B$, $C$, and $D$ are transitively dependent on $A$ (e.g., $A \to B \to C$).
To normalize it, we can decompose it into the relations $R_1[A, B, C]$ and $R_2[C, D]$.
Now, $R_2$ is in 3NF and $R_1$ is in 2NF, but not in 3NF.  We can decompose $R_1$ into the
relations $R_3[A, B]$ and $R_4[B, C]$.  The original relation can be reconstructed by
$\bowtie \left\{ R_2, R_3, R_4 \right\}$.

\paragraph{Illustrative example of data integrity}  Consider a relation of employees.  The
relation contains the attributes ``employee'', ``department'', and ``department head''.
The primary key is ``employee''.  The functional dependencies are ``employee'' determines
``department'', and ``department'' determines ``department head''.  The relation is in 2NF
(with a single-attribute key, partial dependency is impossible) but not 3NF because
``department head'' is transitively dependent on ``employee'' through ``department''.

\begin{tablebox}[label=tab:employee-illustration]{Employee relation.}
  \centering
  \rowcolors{2}{black!10!white}{}
  \begin{tabular}{ccc}
    \toprule
    \textbf{Employee} & \textbf{Department} & \textbf{Department head} \\
    \midrule
    Alice & Engineering & Eve \\
    Bob & Engineering & Eve \\
    Carol & Marketing & Frank \\
    Dan & Marketing & Frank \\
    \bottomrule
  \end{tabular}
  \tcblower
  An example of a relation of employees in 2NF.
\end{tablebox}

\Cref{tab:employee-illustration} shows an example of possible values of the relation.
If we decide to change the head of ``Engineering'' to ``Grace'', we must update
two rows; otherwise, the relation will be inconsistent.  A 3NF relation (see
\cref{tab:employee-illustration2}) would have the attributes ``department'' and ``department
head'' in a separate relation, avoiding the possibility of data inconsistency.  If
needed, the relation would be reconstructed by a join operation.

\begin{tablebox}[label=tab:employee-illustration2]{Employee relation in 3NF.}
  \centering
  \rowcolors{2}{black!10!white}{}
  \begin{tabular}{cc}
    \toprule
    \textbf{Department} & \textbf{Department head} \\
    \midrule
    Engineering & Eve \\
    Marketing & Frank \\
    \bottomrule
  ...
  \end{tabular}
  \tcblower
  An example of a relation of employees in 3NF.
\end{tablebox}
...
$c'$, and product $b$ is in their portfolio, then assuming both companies make $b$, $a$
...
\subsubsection{A single observational unit is stored in multiple tables}
Consider \cref{tab:messy5a,tab:messy5b}, adapted from the nycflights13
dataset\footfullcite{Wickham2022NYC}.  During data collection, a single observational unit
is commonly stored in multiple tables.  Usually, the table (or file) itself represents the
value of a variable.  When columns are compatible, it is straightforward to combine the
tables.
