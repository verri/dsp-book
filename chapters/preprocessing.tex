...
In this chapter, we discuss the \gls{datapreprocessing}, which is the process of adjusting the
data to make it suitable for a particular \gls{learningmachine} or, at the least, to ease the
learning process.

Similarly to data handling, \gls{datapreprocessing} is done by applying a series of operations
to the data.  However, some of the parameters of the operations are not fixed but rather
are fit from a data sampling.  In the context of \gls{inductivelearning}, the sampling is the
training set.
...
I present the most common \gls{datapreprocessing} tasks in three categories: data cleaning,
...
application of the \gls{preprocessor} in production.

Finally, I discuss the importance of the default behavior of the model when the
preprocessing chain degenerates over a sample, i.e. when the \gls{preprocessor} decides that it
...
\begin{mainbox}{Chapter remarks}
  ...
  \begin{itemize}
    ...
    \item Parameters of the \gls{preprocessor} are fitted rather than being fixed.
  \end{itemize}
  ...
  \begin{itemize}
    \itemsep0em
    \item Understand the main \gls{datapreprocessing} tasks and techniques.
  ...
  \begin{itemize}
    \itemsep0em
    \item Each learning method requires specific \gls{datapreprocessing} tasks.
    \item Fitting the \gls{preprocessor} is crucial to avoid \gls{leakage}.
...
learning tasks but also the \gls{datapreprocessing} tasks, which are dependent on the
chosen machine learning methods.

\begin{defbox}{Data preprocessing}{preprocessing}
  The process of adjusting the data to make it suitable for a particular \gls{learningmachine}
...
A \gls{datapreprocessing} strategy $F$ is a function that takes a table $T = (K, H, c)$ and
returns an adjusted table $T' = (K', H', c')$ and a fitted \gls{preprocessor} $f(z; \phi)
...
The objective of the fitted \gls{preprocessor} is to adjust the data to make it suitable for the
...
different pair of \gls{preprocessor} and model.  Sometimes, the developer can choose to
integrate this as an error or warning in the user application.

\subsection{Data preprocessing tasks}

The most common \gls{datapreprocessing} tasks can be divided into three categories:
\begin{itemize}
  \itemsep0em
  \item Data cleaning;
...
In the next sections, I address some of the most common \gls{datapreprocessing} tasks
...
process.  Usually, such errors and inconsistencies make the \glspl{learningmachine} ``confused''
...
There are a few, but important, tasks to be done during \gls{datapreprocessing} in terms of
...
\begin{tablebox}[label=tab:unit-conversion]{Unit conversion preprocessing task.}
  ...
  \begin{tabular}{lp{6cm}}
    ...
    \textbf{Applying} &
      \Gls{preprocessor} converts the numerical values and drops the unit of measurement column.  \\
    \bottomrule
  \end{tabular}
\end{tablebox}
...
\begin{tablebox}[label=tab:range-check]{Range check preprocessing task.}
  ...
  \begin{tabular}{lp{6cm}}
    ...
    \textbf{Applying} &
      \Gls{preprocessor} checks whether the value $x$ of a variable is within the range $[a,
...
\begin{tablebox}[label=tab:text-standardization]{Category standardization preprocessing task.}
  ...
  \begin{tabular}{lp{6cm}}
    ...
    \textbf{Adjustment} &
      Training set is adjusted sample by sample, independently. \\
    \textbf{Applying} &
      \Gls{preprocessor} replaces the categorical variable $x$ with the mapped
...
same issues.  Having the fixes programmed into the \gls{preprocessor} makes it easier to
...
\begin{tablebox}[label=tab:iqr-outlier]{Outlier detection using the interquartile range.}
  ...
  \begin{tabular}{lp{6cm}}
    ...
    \textbf{Adjustment} &
      Training set is adjusted sample by sample, independently. \\
    \textbf{Applying} &
      \Gls{preprocessor} replaces the outlier values with missing data. \\
    \bottomrule
  \end{tabular}
\end{tablebox}
...
enough to fit the parameters of the \gls{preprocessor}.  The reason is that descriptive methods
...
parameters of the \gls{preprocessor} that detects outliers.  Thus, any new data point can
...
\begin{tablebox}[label=tab:outlier-removal]{Task of filtering outliers.}
  ...
  \begin{tabular}{lp{6cm}}
    ...
    \textbf{Fitting} &
      Parameters of the outlier \gls{classifier}. \\
    \textbf{Adjustment} &
      Training set is adjusted sample by sample, independently, removing
      degenerated samples. \\
    \textbf{Applying} &
      \Gls{preprocessor} degenerates if the sample is classified as an outlier and does
      nothing, otherwise. \\
    \bottomrule
  \end{tabular}
\end{tablebox}
...
\begin{tablebox}[label=tab:row-removal-missing]{Task of filtering rows based on missing data.}
  ...
  \begin{tabular}{lp{6cm}}
    ...
    \textbf{Adjustment} &
      ...
      degenerated samples. \\
    \textbf{Applying} &
      \Gls{preprocessor} degenerates over the rows with missing data in the specified variables.
      \\
    \bottomrule
  \end{tabular}
\end{tablebox}
...
\gls{preprocessor} just learns to drop the columns that have missing data during fitting.
...
\begin{tablebox}[label=tab:col-drop-missing]{Task of dropping columns based on missing data.}
  ...
  \begin{tabular}{lp{6cm}}
    ...
    \textbf{Applying} &
      \Gls{preprocessor} drops the chosen columns in fitting. \\
    \bottomrule
  \end{tabular}
\end{tablebox}
...
\begin{tablebox}[label=tab:imputation]{Task of imputing missing data.}
  ...
  \begin{tabular}{lp{6cm}}
    ...
    \textbf{Adjustment} &
      Training set is adjusted sample by sample, independently. \\
    \textbf{Applying} &
      \Gls{preprocessor} replaces the missing values with the chosen statistic. If an indicator
...
However, note that the \gls{preprocessor} for this task \emph{must never do anything with the
...
\begin{tablebox}[label=tab:random-sampling]{Task of random sampling.}
  ...
  \begin{tabular}{lp{6cm}}
    ...
    \textbf{Applying} &
      Pass-through: \gls{preprocessor} does nothing with the new data. \\
    \bottomrule
  \end{tabular}
\end{tablebox}
...
\gls{preprocessor} degenerates over the samples that do not satisfy the rule.  A summary of the
...
\begin{tablebox}[label=tab:scope-filtering]{Task of filtering the scope of the data.}
  ...
  \begin{tabular}{lp{6cm}}
    ...
    \textbf{Adjustment} &
      ...
      samples. \\
    \textbf{Applying} &
      \Gls{preprocessor} degenerates over the samples that do not satisfy the rule. \\
    \bottomrule
  \end{tabular}
\end{tablebox}
...
In this case, the \gls{preprocessor} does not degenerate over the samples, but rather the
...
data, like the naïve Bayes \gls{classifier}.
...
In any case, the \gls{preprocessor} for this task \emph{must never do anything with the new
...
\begin{tablebox}[label=tab:class-balancing]{Task of class balancing.}
  ...
  \begin{tabular}{lp{6cm}}
    ...
    \textbf{Fitting} &
      User must declare the number of samples in each class or
      they are calculated from the training set.  \\
    \textbf{Adjustment} &
      Rows of the training set are randomly chosen. \\
    \textbf{Applying} &
      Pass-through: \gls{preprocessor} does nothing with the new data. \\
...
\begin{tablebox}[label=tab:one-hot]{One-hot encoding preprocessing task.}
  ...
  \begin{tabular}{lp{6cm}}
    ...
    \textbf{Adjustment} &
      Training set is adjusted sample by sample, independently. \\
    \textbf{Applying} &
      \Gls{preprocessor} creates a new column for each possible value of the categorical
...
\begin{tablebox}[label=tab:binning]{Binning numerical values preprocessing task.}
  ...
  \begin{tabular}{lp{6cm}}
    ...
    \textbf{Adjustment} &
      Training set is adjusted sample by sample, independently. \\
    \textbf{Applying} &
      \Gls{preprocessor} converts each numerical value to a categorical value by checking
...
\begin{tablebox}[label=tab:standardization]{Standardization preprocessing task.}
  ...
  \begin{tabular}{lp{6cm}}
    ...
    \textbf{Adjustment} &
      Training set is adjusted sample by sample, independently. \\
    \textbf{Applying} &
      \Gls{preprocessor} scales the values according to \cref{eq:standardization}. \\
    \bottomrule
  \end{tabular}
\end{tablebox}

In the case of rescaling, during production, the \gls{preprocessor} usually clamps\footnote{The
...
\begin{tablebox}[label=tab:rescaling]{Rescaling preprocessing task.}
  ...
  \begin{tabular}{lp{6cm}}
    ...
    \textbf{Adjustment} &
      Training set is adjusted sample by sample, independently. \\
    \textbf{Applying} &
      \Gls{preprocessor} scales the values according to \cref{eq:rescaling}. \\
    \bottomrule
  \end{tabular}
\end{tablebox}
...
The \gls{preprocessor}, then, stores the external dataset and the column to join the data.
...
\begin{tablebox}[label=tab:data-enhancement]{Data enhancement preprocessing task.}
  ...
  \begin{tabular}{lp{6cm}}
    ...
    \textbf{Applying} &
      \Gls{preprocessor} enhances the new data with the external information. \\
    \bottomrule
  \end{tabular}
\end{tablebox}
...
Any unstructured data can be transformed into \gls{structureddata}.  We can see this task as a
\gls{datapreprocessing} task.  Techniques like bag of words, word embeddings, and signal (or
image) processing can be seen as preprocessing techniques that transform unstructured data
into \gls{structureddata}, which is suitable for modeling.
...
under preparation and is available for free: \fullcite{Jurafsky2024}.} for a complete
