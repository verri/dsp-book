...
More specifically, we discuss the \gls{inductivelearning} approach, which involves
...
one particular problem --- \gls{inductivelearning} ---, we need to define the scope of the
...
field into two main approaches: \gls{inductivelearning} and \gls{transductivelearning}\footnote{Transduction is the process of obtaining specific knowledge from specific
observations, and it is not the focus of this book.}.

\Gls{inductivelearning} involves deriving general rules from specific observations.  The
...
\begin{figurebox}[label=fig:learning]{Organizational chart of the learning field.}
  ...
  \begin{tikzpicture}
    ...
    \node[below] at (0, -1.0) {\gls{inductivelearning}};
  \end{tikzpicture}
  \tcblower
  ...
  known input data.  \Gls{inductivelearning} is a yet more specific type of learning that
...
learning in opposition to \gls{inductivelearning} --- are out of the scope of this book.
...
Consider a \gls{learningmachine} capable of generating a set of functions, or
\glspl{model}, $f(x; \theta) \equiv f_\theta(x)$, for a set of parametrizations $\theta
...
left aside the density estimation problem, since it is not addressed in the remainder of
the book.)
...
functions. Choosing the \gls{loss}
\begin{equation*}
...
classification error.  The function $f_\theta$, in this case, is called a \gls{classifier}
and $y$ is called the label or class.
...
functions.  The \gls{loss} function is the squared error
\[
...
a \gls{loss} function is the so-called regression.
The estimator $f_\theta$ of the regression, in this case, is called a \gls{regressor}.

\subsection{A few remarks}

These two tasks are quite general and can be applied to a wide range of problems.  The
modeling of the task at hand and choice of the \gls{loss} function are crucial to the success of
the learning process.
...
\gls{learningmachine} described generates models in a \emph{supervised} manner --- i.e., the target
is known for all samples in the training set ---, there are
alternative ways to solve the \gls{inductivelearning} problem, such as the \emph{semisupervised}
...
Any \gls{learningmachine} produces a model that describes the relationship between the input
...
\emph{one-versus-all}, where we train $l$ binary \glspl{classifier}, one for each class,
and the class with the highest score is the predicted class.  Another method is the
\emph{one-versus-one} method, where we train $l(l-1)/2$ binary \glspl{classifier}, one for each
...
output as well.  The \gls{learningmachine} must be able to handle the input and output data
...
\gls{classifier}}, which minimizes the probability of classification error.  The Bayes
\gls{classifier} is defined as
\begin{equation*}
  f_\text{Bayes}(x) = \argmax_{y \in \mathcal{Y}} \Prob(y \mid x)\text{.}
\end{equation*}

We can easily see that the Bayes \gls{classifier} is the optimal solution for the binary data
classification task.  The probability of classification error for an arbitrary \gls{classifier}
$f$ is
\begin{equation*}
...
a \gls{classifier} that $f(x) = 1$ if $b(x) > 1 - b(x)$ and $f(x) = 0$ otherwise.  This is the
Bayes \gls{classifier}.

Consequently, the \gls{bayeserrorrate}, or irreducible error, is the lowest possible \gls{loss} for any
\gls{classifier} in a given problem.  The \gls{bayeserrorrate} sums the errors of the
Bayes \gls{classifier} for each class:
\begin{equation*}
...
Thus, the \gls{bayeserrorrate} can be rewritten as
\begin{equation*}
  R_\text{Bayes} = \int \min\left\{ b(x), 1 - b(x) \right\}\, d\!\Prob(x)\text{.}
\end{equation*}

\begin{figurebox}[label=fig:bayes-classifier]{Bayes \gls{classifier} illustration.}
  \centering
  \begin{tikzpicture}
    \begin{axis}[
        ticks=none,
  ...
  \end{tikzpicture}
  \tcblower
  The Bayes \gls{classifier} is the line that separates the two classes.  The Bayes error is a
  result of the darker area in which the distributions of the classes intersect.
\end{figurebox}

\Cref{fig:bayes-classifier} illustrates the Bayes \gls{classifier} and its error rate.
The vertical line represents the Bayes \gls{classifier} that separates the classes the best way
...
\gls{classifier} cannot always predict the class correctly.
...
It is easy to show that the \gls{regressionfunction} minimizes the risk \eqref{eq:risk} with
\gls{loss}
\[
...
Like the Bayes \gls{classifier}, the stochastic nature of the data leads to an irreducible
...
\begin{figurebox}[label=fig:explained-unexplained-variance]{Unexplained variance is the
  ...
  \end{tikzpicture}
  \tcblower
  Expected error of the \gls{regressionfunction} for data generated by
  $\Prob(y \mid x) \Prob(x)$ such that $\Prob(y \mid x) = \mathcal{N}(x, 1)$ and
  $\Prob(x) = \mathcal{U}(0, 1)$.  The \gls{regressionfunction} is $r(x) = x$.
\end{figurebox}

\Cref{fig:explained-unexplained-variance} illustrates the irreducible error of the
\gls{regressionfunction} for arbitrary distributions $\Prob(y \mid x)$ and $\Prob(x)$.
In this case, $\E\!\left[ \Var(y \mid x) \right] = 1$, which means that points drawn
by $\Prob(x, y)$ are distributed around the \gls{regressionfunction} $r(x)$ with a standard
...
and $L(z, \theta)$ be a generic \gls{loss} function for the model $f_\theta$.  Note that the
...
the \gls{empiricalrisk} functional
\begin{equation}
  \label{eq:empirical-risk}
  R_n(\theta) = \frac{1}{n} \sum_{i=1}^n L(z_i, \theta)\text{.}
\end{equation}

Approximating $R(\theta)$ by the \gls{empiricalrisk} functional $R_n(\theta)$ is the so-called
...
all realizations of the ERM principle for specific \gls{loss} functions and hypothesis spaces.
...
Consistency means that, given a sufficient number of samples, the \gls{empiricalrisk}
...
later translated to English in \fullcite{Vapnik1991}.} of the \gls{empiricalrisk} functional
...
converges to $R(\theta)$ as the sample size $n$ increases.  It is possible for a \gls{learningmachine}
to be consistent but have a slow convergence rate, which means that a large number of
samples is needed to achieve a good solution.

The asymptotic rate of convergence of the \gls{empiricalrisk} functional $R_n(\theta)$ is
...
Let $L(z, \theta)$, $\theta \in \Theta$, be a set of bounded \gls{loss} functions, i.e.
\[
...
\]
Once the \gls{loss} functions are bounded, this set of vectors belongs to a $n$-dimensional
...
If $L(z, \theta)$, $\theta \in \Theta$, is a set of indicator functions (i.e., the \gls{loss} in a binary
...
related to the nonfalsifiability of the \gls{learningmachine}.  For instance, the set of
...
it implies that the minimum of the \gls{empiricalrisk} is zero independently of the value of
the true risk.
...
\]
is the \gls{growthfunction} of the hypothesis space $\Theta$.

\textcite{Vapnik1968}\footfullcite{Vapnik1968} showed that the \gls{growthfunction} either
satisfies
\[
  G(n; \Theta) = n \ln 2
\]
...
\]
where $h$ is an integer.  Thus, the \gls{growthfunction} is either linear or logarithmic in
$n$.  In the first case, we say that the VC dimension of the hypothesis space is
...
learning process.  By generalization ability, we mean the ability of the \gls{learningmachine}
...
generalization ability of \glspl{learningmachine} in the following way\footnote{ For the sake of
the arguments, we consider only the expression for bounded losses and a hypothesis space
with infinite number of functions.  Rigorously, the \gls{loss} function may not be bounded;
...
\]
where $B$ is the upper bound of the \gls{loss} function, $h$ is the VC dimension of the
...
It is easy to see that as the number of samples $n$ increases, the \gls{empiricalrisk}
...
of the \gls{learningmachine}.

In other words, if $\nicefrac{n}{h}$ is small, a small \gls{empiricalrisk} does not guarantee a small value
...
\begin{tablebox}[label=tab:overfit-underfit]{Overfitting and underfitting.}
  ...
  \begin{tabular}{lll}
    \toprule
    \textbf{Problem} & \textbf{\Gls{empiricalrisk}} & \textbf{Confidence interval} \\
    \midrule
  ...
  the complexity of the training data (high \gls{empiricalrisk}).  Overfitting occurs when the
  model is too complex (high VC dimension increases the confidence interval) and fits the
  training data almost perfectly (low \gls{empiricalrisk}).
\end{tablebox}
...
The \gls{srm} principle consists of minimizing both the \gls{empiricalrisk} (optimizing the
...
of each set $S_k$.  This is called an \gls{admissiblestructure}.
...
\begin{figurebox}[label=fig:srm-tradeoff]{SRM trade-off.}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot[smooth] {(x - 1)^2 + 0.1} node[right, text width=2cm] {\Gls{empiricalrisk}};
  ...
  \end{tikzpicture}
  \tcblower
  The upper bound of the risk is the sum of the \gls{empiricalrisk} and the confidence
  interval.  The smallest bound is found for some $k^*$ in the \gls{admissiblestructure}.
\end{figurebox}

Given the observations $z_1, \dots, z_n$, the \gls{srm} principle chooses a function
$L(z, \theta_n^k)$ that minimizes the \gls{empiricalrisk} $R_n(\theta_n^k)$ in the subset
...
\gls{biasvariance}.  The \gls{biasvariance} is a well-known concept in
...
Consider a \gls{learningmachine} that produces a function $\hat{f}(x; D)$ based on the
...
\begin{align}
  \notag \int \big( y - \hat{f}(x) \big)^2\, d\!\Prob(D, \epsilon)
    ...
    &= \label{eq:term1}\int \big( y - f(x) \big)^2\, d\!\Prob(\epsilon) \\
...
\begin{equation}
  ...
  \int \big( y - f(x) \big)^2\, d\!\Prob(\epsilon) =
  \int \big( f(x) + \epsilon - f(x) \big)^2\, d\!\Prob(\epsilon) =
  \int \epsilon^2\, d\!\Prob(\epsilon) = \sigma^2\text{.}
\end{equation}
...
model acquired by the \gls{learningmachine} and that we observe in the training data --- i.e.
the \gls{empiricalrisk}.  The variance error, which does not depend on the real function $f$
but on the models the \gls{learningmachine} can generate, is the error that comes from how
different the models can be from each other --- i.e. the confidence interval that comes from
the VC dimension.

\subsection{Regularization}

Also related to the \gls{srm} principle is the concept of \gls{regularization}.
\Gls{regularization} encourages models to learn robust patterns within the data rather than
memorizing it.

\Gls{regularization} techniques usually modify the \gls{loss} by adding a penalty term that
depends on the complexity of the model.  So, instead of minimizing the \gls{empiricalrisk}
$R_n(\theta)$, the \gls{learningmachine} minimizes the regularized \gls{empiricalrisk}
\[
  R_n(\theta) + \lambda \Omega(\theta)\text{,}
\]
where $\Omega(\theta)$ is the complexity of the model and $\lambda$ is a hyperparameter
that controls the trade-off between the \gls{empiricalrisk} and the complexity.
Note that the \gls{regularization} term acts as a proxy for the confidence interval in the
\gls{srm} principle.  However, \gls{regularization} is often justified by common sense or
...
stopping, dropout, ensembles, and pruning --- are often called implicit \gls{regularization}.
...
We show two \glspl{learningmachine} that implement the \gls{srm} principle in different ways:
\begin{itemize}
  \itemsep0em
  \item The perceptron, which fixes the complexity of the model and tries to minimize the
    \gls{empiricalrisk}; and
  \item The maximal margin classifier, which fixes the \gls{empiricalrisk} --- in this case,
...
The perceptron is a linear \gls{classifier} that generates a hyperplane that separates the
classes in the feature space.  It is a parametric model, and the learning process minimizes the \gls{empiricalrisk}
by adjusting its fixed set of parameters.

\begin{defbox}{Parametric model}{parametric}
  If the \gls{learningmachine} generates a set of functions $f_\theta$ where the number of
...
effectively minimizing the \gls{empiricalrisk}.  The algorithm updates the weights iteratively
...
\gls{empiricalrisk} using the gradient descent algorithm while controlling the complexity of
the model with \gls{regularization} techniques\footnote{To counterbalance the potential
...
We saw that the perceptron tries to minimize the \gls{empiricalrisk}, but it makes no effort to
reduce the confidence interval.  A different approach would be to fix the \gls{empiricalrisk}
...
Before we dive into the \gls{classifier} that minimizes the confidence interval, consider the
...
$\Delta$ of the hyperplane.  The \emph{maximal margin classifier} is a \gls{learningmachine}
...
the maximal margin classifier is that the separating hyperplane is built from the \glspl{supportvector}
--- the vectors that are exactly in the margin.  In \cref{fig:maximal-margin-and}, the
\glspl{supportvector} are the points $(1, 0)$, $(0, 1)$, and $(1, 1)$.

In other words, maximal margin classifier is
\[
  f(x) = \sign\left(\sum_{i=1}^{n} y_i a_i \big(\vec{x}_i \cdot x\big) - b\right)\text{,}
\]
for some $b$ and coefficients $a_i > 0$ for the \glspl{supportvector} ($a_i = 0$ otherwise).

In the case that the classes are not linearly separable, the maximal margin classifier can
be extended to the \emph{soft margin classifier}, which sets the \gls{empiricalrisk} to a value
greater than zero.

Moreover, since the number of parameters of the maximal margin classifier depends on the
training data (i.e., the number of \glspl{supportvector}), it is a nonparametric model.
...
\glspl{learningmachine}.  The principle not only explains many of the empirical results in
\gls{ml} but also provides a theoretical framework to guide the development of new
\glspl{learningmachine}.
...
the theoretical principles behind them.
% Some comments about a few methods are given in
