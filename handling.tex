...
\begin{definition}{Indexed table}\label{def:itable}
...
\begin{definition}{Nested row}\label{nested-row}
A \emph{nested
...
\begin{definition}{Value matrix}\label{def:vmatrix}
...
\begin{definition}{Split operation}\label{def:split}
...
\begin{definition}{Disjoint tables}\label{def:disjoint-tables}
...
\begin{definition}{Bind operation}\label{def:bind}
...
\begin{definition}{Split invariance}\label{def:split-invariance}
...
\begin{enumerate}
  \itemsep0em
  \item We can safely assume that Alice has never taken the Biology class, as $\text{Chemistry}
...
\begin{figure}
  ...
  \end{tikzpicture}
  \caption{
  Example of data handling pipeline.
...
\begin{definition}{Tagged bind operation}\label{def:tagged-bind}
...
\begin{definition}{Tagged split operation}\label{def:tagged-split}
...
\begin{definition}{Pivot long-to-wide operation}\label{def:pivot-l2w}
...
\begin{definition}{Pivot wide-to-long operation}\label{def:pivot-w2l}
...
\begin{definition}{Join operation}\label{def:join}
...
In our context, this operation is an unary operation, where the second table is
a fixed parameter.

\begin{definition}{Left join operation}\label{def:left-join}
  Let $T' = (K', H', c')$ and $T'' = (K'', H'', c'')$ be two tables such that $K'
...
\begin{definition}{Selection operation}\label{def:selection}
...
\begin{definition}{Predicate selection operation}\label{def:predicate-selection}
...
\begin{definition}{Filtering operation}\label{def:filtering}
...
\begin{definition}{Mutation operation}\label{def:mutating}
...
\begin{definition}{Aggregation operation}\label{def:aggregating}
...
We discussed that the less index columns a table has ---
...
\begin{definition}{Ungrouping operation}\label{def:grouping}
...
\begin{definition}{Projection operation}\label{def:projection}
...
\begin{table}
  ...
  \label{tab:student-grade-handling}
\end{table}
...
\begin{figure}
  ...
  \label{fig:mutating-groups-order}
\end{figure}
...
Note that statistical data handling differ from relational algebra, because the former focus
