...
\begin{defbox}{Filtering operation}{filtering}
  Let $T = (K, H, c)$ be a table and $P_1, \dots P_n$ be predicates.  The filtering
  operation is defined as \[
    \operatorname{filter}_{P_1, \dots, P_n}(T) = T'\text{,}
  \] where $T' = (K, H, c')$ and \[
    c'(r, h) = \begin{dcases}
      c(r, h) & \text{if } \bigvee_{i = 1}^{n} P_i(V(r)) = 1\text{,} \\
      () & \text{otherwise}\text{,}
    \end{dcases}
  \] where predicate $P_i : (\domainof{h} \times \dots : h \in H) \to \{0, 1\}$ is applied
  to the value matrix $V(r)$ of the row $r$.
\end{defbox}

For instance, the predicate \code{age > 18} will select all rows where the value in the
age column is greater than 18.

Keeping each row independent, we can also generalize predicates to deal with the values of
the indexes as well.

\begin{defbox}{Indexed filtering operation}{indexed-filtering}
  Let $T = (K, H, c)$ be a table and $P_1, \dots P_n$ be (indexed) predicates.  The
  indexed filtering operation is defined as \[
    \operatorname{filter}_{P_1, \dots, P_n}(T) = T'\text{,}
  \] where $T' = (K, H, c')$ and \[
    c'(r, h) = \begin{dcases}
      ...
      () & \text{otherwise}\text{,}
    \end{dcases}
  \] where predicate $P_i : (\domainof{K_i} \times \dots : K_i \in K ) \times
  (\domainof{h} \times \dots : h \in H) \to \{0, 1\}$ is applied to the value matrix
  $V(r)$ of the row $r$.
\end{defbox}
...
\subsection{Filtering rows after aggregation}
