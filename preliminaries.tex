...
\begin{algobox}[label=alg:bsearch]{Binary search algorithm.}
  \KwData{A sorted array $A = \left[a_1, a_2, \dots, a_n\right]$ and a key $x$}
  \KwResult{True if $x$ is in $A$, false otherwise}
  $l \gets 1$\;
  $r \gets n$\;
  \While{$l \leq r$}{
...
One example of suboptimal greedy algorithm is the naïve solution for the knapsack problem.
...
\begin{algobox}[label=alg:knapsack]{Naïve solution for the knapsack problem.}
  ...
  \Return{$x_i,~\forall i$}
  \tcblower
  A naïve greedy algorithm that solves the knapsack problem.  The algorithm iterates over
  the items in decreasing order of value and puts the item in the knapsack if it fits.
\end{algobox}

A naïve greedy algorithm for the knapsack problem is shown in \cref{alg:knapsack}.  The
...
are the naïve solutions for N-queens problem and the Sudoku problem.  Backtracking usually
leads to exponential time complexity --- see more in the following.
